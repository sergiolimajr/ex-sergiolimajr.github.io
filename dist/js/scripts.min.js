...
class Selecao{constructor(e,t){this.destino=e,this.menor=t}}var menorValor=new Selecao(null,999999),arrayDistances=[];let btnSearch=document.getElementById("getDistance");function tratarRetorno(e,t){let n=document.getElementById("map");t!=google.maps.DistanceMatrixStatus.OK?sResult.innerHTML=t:(sResult.innerHTML=` <b>Origem:</b> ${e.originAddresses} \n                          <br /><b>Destino:</b> ${e.destinationAddresses}\n                          <br /><b>Distância:</b> ${e.rows[0].elements[0].distance.text}\n                          <br /><b>Duração:</b> ${e.rows[0].elements[0].duration.text}\n                          <br /><b>Tipo de Transporte:</b> Carro`,n.src=`https://maps.google.com/maps?saddr=${e.originAddresses}&daddr=${e.destinationAddresses}&output=embed`),t===google.maps.DistanceMatrixStatus.OK&&(arrayDistances.push(e.rows[0].elements[0].distance.text),e.rows[0].elements[0].distance.value<menorValor.menor&&(menorValor.destino=e.destinationAddresses,menorValor.menorValor=e.rows[0].elements[0].distance.value))}function createNode(e){return document.createElement(e)}function append(e,t){return e.appendChild(t)}btnSearch.addEventListener("click",function(){let e=document.getElementById("origem"),t=document.getElementById("destino");sResult=document.getElementById("search-result"),sResult.innerHTML="Aguarde...",(new google.maps.DistanceMatrixService).getDistanceMatrix({origins:[e.value],destinations:[t.value],travelMode:google.maps.TravelMode.DRIVING,unitSystem:google.maps.UnitSystem.METRIC},tratarRetorno)}),navigator.serviceWorker&&navigator.serviceWorker.register("/service-worker.js").then(function(e){console.log("ServiceWorker registration successful with scope:",e.scope)}).catch(function(e){console.log("ServiceWorker registration failed:",e)});const divEmergencia=document.getElementById("emergencia"),URL_EMERGENCIA="https://raw.githubusercontent.com/sergiolimajr/projeto-integrador/master/json/emergencia.json";fetch(URL_EMERGENCIA).then(e=>e.json()).then(e=>{return e.services.map(e=>{let t=createNode("div"),n=createNode("a");t.innerHTML=`${n}`,n.innerHTML=`${e.name}`,n.href="../../dist/view/serviceContent.html",n.className="collection-item waves-effect black-text",t.className="collection",append(divEmergencia,t),append(t,n)})}).catch(e=>{console.log("Falha na recuperação de informações",e)});const divUrgencia=document.getElementById("urgencia"),URL_URGENCIA="https://raw.githubusercontent.com/sergiolimajr/projeto-integrador/master/json/urgencia.json";fetch(URL_URGENCIA).then(e=>e.json()).then(e=>{return e.services.map(e=>{let t=createNode("div"),n=createNode("a");t.innerHTML=`${n}`,n.innerHTML=`${e.name}`,n.href="../../dist/view/serviceContent.html",n.className="collection-item waves-effect black-text",t.className="collection",append(divUrgencia,t),append(t,n)})}).catch(e=>{console.log("Falha na recuperação de informações",e)}),$(document).ready(function(){$(".sidenav").sidenav(),$(".tooltipped").tooltip(),$(".dropdown-trigger").dropdown(),$(document).on("pageload",function(){$(".modal-trigger").leanModal({dismissible:!0,opacity:.5,complete:function(){$(document).trigger("modalclose")}})})});